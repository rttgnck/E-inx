...
// X3 "half" scrub bank, from the FreeInk/YACP UC8253 X3 port (Uc8253X3Luts.h).
// WW == BW and WB == BB, so the transition a pixel receives depends only on where it
// is going and not at all on where it came from. That is what lets a cleanup reset
// accumulated differential error without first flashing the panel to white, which the
// _img full sync does. Loader CDI is 0xA9,0x07. Keep these bytes verbatim.
const uint8_t lut_x3_vcom_half[] PROGMEM = {0x00, 0x06, 0x01, 0x06, 0x06, 0x01, 0x00, 0x04, 0x01, 0x01, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                            0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
const uint8_t lut_x3_ww_half[] PROGMEM = {0xAA, 0x06, 0x01, 0x06, 0x06, 0x01, 0xA0, 0x04, 0x01, 0x01, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                          0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                          0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
const uint8_t lut_x3_bw_half[] PROGMEM = {0xAA, 0x06, 0x01, 0x06, 0x06, 0x01, 0xA0, 0x04, 0x01, 0x01, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                          0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                          0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
const uint8_t lut_x3_wb_half[] PROGMEM = {0x55, 0x06, 0x01, 0x06, 0x06, 0x01, 0x50, 0x04, 0x01, 0x01, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                          0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                          0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
const uint8_t lut_x3_bb_half[] PROGMEM = {0x55, 0x06, 0x01, 0x06, 0x06, 0x01, 0x50, 0x04, 0x01, 0x01, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                          0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                          0x00, 0x00, 0x00, 0x00, 0x00, 0x00};

// X3 "fast" differential bank, from the same source. This is the waveform YACP runs on
// an ordinary page turn; E-inx has always used the _full bank there instead. Exposed
// through Settings so the two can be compared on hardware rather than argued about.
// Loader CDI is 0x29,0x07. Keep these bytes verbatim.
const uint8_t lut_x3_vcom_fast[] PROGMEM = {0x00, 0x04, 0x02, 0x04, 0x04, 0x01, 0x00, 0x04, 0x01, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                            0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
const uint8_t lut_x3_ww_fast[] PROGMEM = {0x20, 0x04, 0x02, 0x04, 0x04, 0x01, 0x00, 0x04, 0x01, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                          0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                          0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
const uint8_t lut_x3_bw_fast[] PROGMEM = {0xAA, 0x04, 0x02, 0x04, 0x04, 0x01, 0x80, 0x04, 0x01, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                          0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                          0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
const uint8_t lut_x3_wb_fast[] PROGMEM = {0x55, 0x04, 0x02, 0x04, 0x04, 0x01, 0x40, 0x04, 0x01, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                          0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                          0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
const uint8_t lut_x3_bb_fast[] PROGMEM = {0x10, 0x04, 0x02, 0x04, 0x04, 0x01, 0x00, 0x04, 0x01, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                          0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                          0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
...
void EInkDisplay::displayBuffer(RefreshMode mode, const bool turnOffScreen) {
  ...
  if (_x3Mode) {
    ...
    if (doFullSync) {
      ...
      // Fast differential: RED RAM (0x10) retains previous frame. Which bank drives it is a
      // setting, because the one E-inx has always used here is not the one YACP uses.
      const bool yacpFast = _x3PageWaveform == 2;
      sendCommandDataX3(0x20, yacpFast ? lut_x3_vcom_fast : lut_x3_vcom_full, 42);
      sendCommandDataX3(0x21, yacpFast ? lut_x3_ww_fast : lut_x3_ww_full, 42);
      sendCommandDataX3(0x22, yacpFast ? lut_x3_bw_fast : lut_x3_bw_full, 42);
      sendCommandDataX3(0x23, yacpFast ? lut_x3_wb_fast : lut_x3_wb_full, 42);
      sendCommandDataX3(0x24, yacpFast ? lut_x3_bb_fast : lut_x3_bb_full, 42);
...
void EInkDisplay::displayHalfScrub(const RefreshMode fallback, const bool turnOffScreen) {
  if (!frameBuffer) return;
  if (!_x3Mode) {
    displayBuffer(fallback, turnOffScreen);
    return;
  ...
  // The scrub itself needs no baseline — WW == BW and WB == BB, so DTM1 is not consulted.
  // But an explicit resync request, or the boot sequence, is asking for a real full sync and
  // should get one rather than being quietly downgraded.
  if (_x3ForceFullSyncNext || _x3InitialFullSyncsRemaining > 0) {
    displayBuffer(fallback, turnOffScreen);
    return;
  }
  ...
  if (Serial) Serial.printf("[%lu]   X3_OEM_HALF_SCRUB\n", millis());
  ...
  if (!isScreenOn) {
    sendCommand(0x04);
    waitForRefresh(" X3_CMD04(scrub)");
    isScreenOn = true;
  }
  sendCommand(0x12);
  waitForRefresh(" X3_CMD12(scrub)");

  if (turnOffScreen) {
    sendCommand(0x02);
    waitForRefresh(" X3_CMD02_POWEROFF(scrub)");
    isScreenOn = false;
  }
