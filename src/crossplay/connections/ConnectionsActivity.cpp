...
constexpr char kFetchScratch[] = "/.einx/crossplay/connections.dl";
...
void ConnectionsActivity::beginImport() {
  ...
  crossplay::chooseWifi(*this, renderer.writable(), mappedInput,
...
void ConnectionsActivity::runImport() {
  ...
  const bool fetched =
      crossplay::fetchUrlStreaming(kArchiveUrl, kFetchScratch, [this, &importer, &lastPaint](const uint8_t* data, const size_t len) {
  ...
  switch (view) {
    case View::Board: {
      ...
      // ceiling: the chrome speaks Jersey, the tiles speak the game's serif.
      {
        ...
        target.setFont(einxui::EInxDrawTarget::FONT_BODY, toybox::kSerifSmallFontId);
